...
\begin{itemize}
    \item Representing an area of mathematics as the study of a collection of similar structures for a fixed vocabulary. 
    \item Restriction to \textit{definable} subsets rather than all subsets of the structure.
    \item \textit{Stability theory} highlights combinatorial features that are present in very distinct areas of mathematics. 
\end{itemize} 
The crucial argument is captured by part of Baldwin's theses on contemporary model theory which expands upon the points listed above \cite{Baldwin_2018}
\begin{itemize}
...
I believe the essential features captured by the above for the purpose of mathematical discovery (and designing a conjecturing system) is the focus on \textit{local} formalizations and comparisons between them.  
...
\begin{itemize}
    ...
    \item \textit{Approximation of functions}: Suppose one or more of the $f_i$'s are not definable. When calculating the signum-loss as in \ref{loss},
    on any computing system, we are necessarily restricted to a \textit{finite} number of computations because $\Omega$ will always be finite. In this case, all of the $f_i$'s become discrete.
