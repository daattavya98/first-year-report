...
For the purpose of this appendix I assume some basic terminology from first-order logic and present some definitions and results from model theory. For a detailed introduction and proofs, please see \cite{changModelTheory1990,Dries_1998}.
...
\begin{definition}[Structure]
    \label{def:structure}
    A structure $S$ on the real line consists of a boolean algebra $S_n$ of subsets of $\R^n$ for each $n\in\N$ such that the following hold:
    \begin{enumerate}
        \item The diagonals $\Delta_{ij} := \{x\in\R^n: x_i=x_j\}, 1\leq i<j\leq n$ belong to $S_n$.
        \item $A\in S_m, B\in S_n\implies A\times B\in S_{m+n}$
        \item $A\in S_{n+1}\implies\pi(A)\in S_n$, where $\pi:\R^{n+1}\mapsto\R^n$ is the projection map defined by $\pi(x_1,\dots,x_{n+1}) = (x_1,\dots,x_n)$
        \item The ordering $\{(x,y)\in\R^2: x < y\}$ of the real line belongs to $S_2$
    \end{enumerate}
\end{definition}

\begin{definition}[o-minimal structure]
    \label{def:o-minimal-structure}
    A structure $S$ on the real line is o-minimal if the sets in $S_1$ are exactly the finite unions of intervals and points.
\end{definition}

\begin{definition}[$R_{alg}$]
    \label{def:ralg}
    $R_{alg}$ is the o-minimal structure whose definable sets are precisely the semi-algebraic sets.
\end{definition}
